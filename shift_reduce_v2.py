...
class MultiTask(nn.Module):
    def __init__(self, config, encoder_output_dim, action_dict, ent_dict, tri_dict, arg_dict):
        ...
        self.hidden_ent_corel = _nn.Linear(hidden_input_dim_co, config['output_hidden_dim'],
                                    activation='tanh')
        self.output_ent_corel = _nn.Linear(config['output_hidden_dim'], 2)

        self.position_embed = _nn.Embedding(500, 20)

        attn_input = self.bi_rnn_dim * 1 + 20 * 2
        self.attn_hidden = _nn.Linear(attn_input, 80, activation='tanh')
        self.attn_out = _nn.Linear(80, 1)
    ...
    def forward_ent_corel(self, beta_embed, lmda_embed, sigma_embed, delta_embed, out_embed, hidden_mat, start1, ent1, start2, end2, seq_len, last_h, gold_arg):
        ...
        state_embed = F.dropout(state_embed, p=0.2, training=self.training)
        hidden = self.hidden_ent_corel(state_embed)
        out = self.output_ent_corel(hidden)

        loss = F.cross_entropy(out, gold_arg, reduction='mean')
        return loss
    ...
    def forward_arg(self, beta_embed, lmda_embed, sigma_embed, delta_embed, out_embed, hidden_mat, tri_idx, ent_start, ent_end, seq_len, last_h, gold_arg):
        ...
        state_embed = ops.cat([beta_embed, lmda_embed, sigma_embed, delta_embed, out_embed, attn_rep], dim=0)
        state_embed = F.dropout(state_embed, p=0.25, training=self.training)
    ...
    def position_aware_attn(self, hidden_mat, last_h, start1, ent1, start2, end2, seq_len):
        ...
        tri_pos_emb = self.position_embed(torch.tensor(tri_pos_list, dtype=torch.long))
        tri_pos_mat = tri_pos_emb.unsqueeze(0)
        ent_pos_emb = self.position_embed(torch.tensor(ent_pos_list, dtype=torch.long))
        ent_pos_mat = ent_pos_emb.unsqueeze(0)

        att_input = ops.cat([hidden_mat, tri_pos_mat, ent_pos_mat], dim=0)
        hidden = self.attn_hidden(att_input)
        attn_out = self.attn_out(hidden)
        attn_prob = F.softmax(attn_out, dim=1)
        rep = hidden_mat * attn_prob.transpose(0, 1)
        return rep
...
class ShiftReduce(nn.Module):
    ...
    def __init__(self, config, encoder_output_dim, action_dict, ent_dict, tri_dict, arg_dict):
        ...
        self.lmda_dim = lmda_dim
        self.bi_rnn_dim = bi_rnn_dim
        self.lambda_var = _nn.LambdaVar(lmda_dim)
        
        dp_state = config['dp_state']
        dp_state_h = config['dp_state_h']

        num_layers = 2
        self.sigma_rnn = _nn.StackLSTM(lmda_dim, lmda_dim, dp_state, dp_state_h)  # StackLSTM
        self.delta_rnn = _nn.StackLSTM(lmda_dim, lmda_dim, dp_state, dp_state_h)  # StackLSTM

        self.part_ent_rnn = _nn.StackLSTM(bi_rnn_dim, part_ent_dim, dp_state, dp_state_h)
        #self.beta = []  # buffer, unprocessed words
        self.actions_rnn = _nn.StackLSTM(config['action_embed_dim'], config['action_rnn_dim'], dp_state, dp_state_h)
        self.out_rnn = _nn.StackLSTM(bi_rnn_dim, config['out_rnn_dim'], dp_state, dp_state_h)

        self.act_table = _nn.Embedding(len(action_dict), config['action_embed_dim'])
        self.ent_table = _nn.Embedding(len(ent_dict), config['entity_embed_dim'])
        ...
        self.hidden_arg = _nn.Linear(lmda_dim * 2 + self.bi_rnn_dim, config['output_hidden_dim'],
                                    activation='tanh')
        self.output_arg = _nn.Linear(config['output_hidden_dim'], len(arg_dict))
        self.empty_buffer_emb = None

        self.event_cons = EventConstraint(ent_dict, tri_dict, arg_dict)
        #self.cached_valid_args = self.cache_valid_args(ent_dict, tri_dict)
        self.empty_times = 0
        
    def forward(self, toks, hidden_state_list, last_h, oracle_actions=None, oracle_action_strs=None, is_train=True, ents=None, tris=None, args=None):
        ...
        buffer = _nn.Buffer(self.bi_rnn_dim, hidden_state_list)
        ...
        while not (buffer.is_empty() and self.lambda_var.is_empty() and self.part_ent_rnn.is_empty()):
            ...
            if is_train:
                log_probs = _nn.log_softmax(logits)
            else:
                log_probs = _nn.log_softmax(logits)
            ...
            if self.act.is_o_del(action):
                ...
                self.lambda_var.push(ent_rep, start, _nn.LambdaVar.ENTITY)
