...
class EventConstraint(object):
    ...
    def __init__(self, ent_dict, tri_dict, arg_dict):
        ...
        for cons in self.constraint_list:
            # print(cons)
            # print(f'ent_dict.tok2idx: {ent_dict.tok2idx}')
            # print(f'tri_dict.tok2idx: {tri_dict.tok2idx}')
            # print(f'arg_dict.tok2idx: {arg_dict.tok2idx}')
            
            if cons[0] in ent_dict.keys():
                ent_id = ent_dict[cons[0]]
            else:
                ent_id = ent_dict['*UNK*']

            if cons[1] in tri_dict.keys():
                tri_id = tri_dict[cons[1]]
            else:
                tri_id = tri_dict['*UNK*']

            if cons[2] in arg_dict.keys():
